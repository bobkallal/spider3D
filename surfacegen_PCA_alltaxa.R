...
# these are the files with the points and curves (from IDAV), the coordinate data, the ply mesh from the CT program, and the surface points, the patch, and the mesh list
{
  ...
  skmeshlist<-dir("/carapace/ply",pattern=".ply") # list with link of meshes
  }
...
#joining the surfaces and points
dimnames(ptsarray)[3]<-list(
  substr(dir("/carapace/ply",pattern=".ply"),1,(nchar(dir("/carapace/ply",pattern=".ply"))-4)))###gives specimens names based on file names in ply folder
arraylm<-ptsarray
...
patchedtest250<-placePatch(atlas,ptsarray,path="/carapace/ply",inflate=250)
#checkLM(patchedtest250, path="/carapace/ply/", atlas=atlas)

#slide surface landmarks and curves
setwd("C:/Users/kallal/Desktop/carapace/ply")
slidewithcurves<-slider3d(patchedtest250,SMvector = fixed, outlines = curvein, deselect = TRUE, surp = surface, sur.path="/carapace/ply/", meshlist = skmeshlist, iterations=3,mc.cores=1)
...
car.gpa<-gpagen(newarray) #procrustes align
car.gpa$coords<-car.gpa$coords[-c(253:472),,]

#PCA and plot in morphospace
car.pca<-gm.prcomp(Y.gpa$coords)
...
tree<-read.tree("/carapace/bigtree2.tre")
