...

{
  ...
  template.mesh<-ply2mesh("/paturon/ply_onedip/araneus.ply") #template surface (converted from binary to ascii)
  ...
  skmeshlist<-dir("/paturon/ply_onedip",pattern=".ply") # list with link of mesh (link=directory?)
  }
...
dimnames(ptsarray)[3]<-list(
  substr(dir("/paturon/ply_onedip",pattern=".ply"),1,(nchar(dir("/paturon/ply_onedip",pattern=".ply"))-4)))###gives specimens names based on file names in ply folder
arraylm<-ptsarray
...
#checkLM(ptsarray,path="/paturon/ply_onedip/",suffix=".ply",render="s",alpha=1,begin=1)
...
patchedtest100<-placePatch(atlas,ptsarray,path="/paturon/ply_onedip",inflate=100)
#checkLM(patchedtest100, path="/paturon/ply_onedip/", atlas=atlas)

#slide surface landmarks and curves
setwd("/paturon/ply_onedip")
slidewithcurves<-slider3d(patchedtest100,SMvector = fixed, outlines = curvein, deselect = TRUE, surp = surface, sur.path="/paturon/ply_onedip/", meshlist = skmeshlist, iterations=3,mc.cores=1)
