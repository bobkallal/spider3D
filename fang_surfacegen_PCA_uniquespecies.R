...

{
  ...
  template.mesh<-ply2mesh("/fang/ply/ascii_onedip/araneus.ply") #template surface (converted from binary to ascii)
  ...
  skmeshlist<-dir("/fang/ply/ascii_onedip",pattern=".ply") # list with link of mesh (link=directory?)
  }
...
dimnames(ptsarray)[3]<-list(
  substr(dir("fang/ply/ascii_onedip",pattern=".ply"),1,(nchar(dir("fang/ply/ascii_onedip",pattern=".ply"))-4)))###gives specimens names based on file names in ply folder
arraylm<-ptsarray
...
#checkLM(ptsarray,path="fang/ply/ascii_onedip/",suffix=".ply",render="s",alpha=1,begin=1)
...
#checkLM(patchedtest50, path="fang/ply/ascii_onedip/", atlas=atlas)

#slide surface landmarks and curves
setwd("fang/ply/ascii_onedip")
slidewithcurves<-slider3d(patchedtest100,SMvector = fixed, outlines = curvein, deselect = TRUE, surp = surface, sur.path="fang/ply/ascii_onedip/", meshlist = skmeshlist, iterations=3,mc.cores=1)
...
#PCA geomorph
fang_onedip.gpa<-gpagen(specimens)
